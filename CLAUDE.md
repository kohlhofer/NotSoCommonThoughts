...
Posts live in `src/content/blog/`. The `blog-post` skill (`.agents/skills/blog-post/`, linked
from `.claude/skills/` so Claude and OpenCode both load it) holds the house rules and reads
the latest posts before writing; the voice comes from the user-level `alex-writing-voice`.
